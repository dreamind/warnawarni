...
const generatePalette = (code: string) => {
  ...
  const p4: any[] = []
  config.value = hex.slice(1)
  const luminance = 1000 - (colord(hex).luminance() * 1000);
  // map to 10 - 1000
  config.valueStop = DEFAULT_STOPS.sort( (a, b) => Math.abs(luminance - a) - Math.abs(luminance - b) )[0]  
  ...
  for (let i = 1; i <= 9; i += 1) {
    const name = i + ''
    const r = c5['r' + name] * 255
    const g = c5['g' + name] * 255
    const b = c5['b' + name] * 255
